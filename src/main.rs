...
async fn fetch_headerbomb(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_queryflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_deeppath(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_authflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_cachebypass(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_formmulti(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_xmlbomb(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_graphqlflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_redirectloop(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
...
async fn fetch_emptybody(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_chunkedflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_trailheaders(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_connectionclose(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_expect100(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_varyflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_deflatebomb(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_traceamplify(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_hostpoison(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
    let host = {
        let mut rng = rand::rng();
...
async fn fetch_conditionalflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
    let (ims, inm) = {
        let mut rng = rand::rng();
...
async fn fetch_corsflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
    let origin = {
        let mut rng = rand::rng();
...
async fn fetch_putflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_deleteflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_sessionflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_contenttypeflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
    let ct = {
        let mut rng = rand::rng();
...
async fn fetch_upgradeamplify(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
    let (upgrade, conn) = {
        let mut rng = rand::rng();
...
async fn fetch_headflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_optionsflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_patchflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_slowpost(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_jsonbomb(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_contentnegotiate(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
    let accept = {
        let mut rng = rand::rng();
...
async fn fetch_preferflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
    let prefer = {
        let mut rng = rand::rng();
...
async fn fetch_rangeoverlap(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
    let range = {
        let mut rng = rand::rng();
...
async fn fetch_multipost(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_cspreports(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_connectflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_keepaliveflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_linkflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_forwardedflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_healthflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_jwtexplode(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
    let jwt = {
        let mut rng = rand::rng();
...
async fn fetch_uploadflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_graphqlintrospect(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_adminflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_paramflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_teflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_wantdigestflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_savedataflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_secfetchflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_csvbomb(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_serializedbomb(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_wellknownflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_swaggerflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_loginflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_methodoverrideflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_cookiebomb2(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_graphqlbatch(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_webhookflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_apiversionflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_prototypeflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_jsonpflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_arrayflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_sitemapflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_unicodeflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_paramduplicate(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_cachebusterflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_fileenumflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_soapflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_signedheaderflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_utf8bomflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_doubledotflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_emptyparamflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_headerorderflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
    let user_agents = [
...
async fn fetch_crossdomainflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
...
async fn fetch_refererflood(c: Client, url: String, delay: u64, proxy_idx: usize, sessions: Arc<Vec<std::sync::Mutex<String>>>, verbose: bool, max_retries: usize) -> Result<(usize, u16), FetchError> {
    if delay > 0 { tokio::time::sleep(Duration::from_millis(delay)).await; }
    let referers = [
...
async fn main() {
  ...
  signedheaderflood AWS-style Authorization: AWS4-HMAC-SHA256 headers
